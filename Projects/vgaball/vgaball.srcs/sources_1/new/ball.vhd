...
ARCHITECTURE Behavioral OF ball IS
	CONSTANT size  : INTEGER := 24; -- changed from 8
	...
	SIGNAL ball_x_motion : STD_LOGIC_VECTOR(10 DOWNTO 0) := "00000000100"; -- Add x motion
BEGIN
	red <= NOT ball_on; -- changed from red ball to cyan
	green <= '1';
	blue  <= '1';
	-- process to draw ball current pixel address is covered by ball position
	bdraw : PROCESS (ball_x, ball_y, pixel_row, pixel_col) IS
	BEGIN
	   -- Square to Circle Ball
		...
		BEGIN
			...
			-- allow for bounce of sides of screen
			...
			ball_x <= ball_x + ball_x_motion; -- compute x motion
		END PROCESS;
END Behavioral;